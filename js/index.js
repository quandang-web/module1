...
function hienThiDashboard() {
    ...
    document.getElementById("header-subtitle").textContent = "Tổng quan số liệu thống kê hệ thống";
...
function hienThiHome(listStudent) {
    ...
    document.getElementById("header-subtitle").textContent = "Xem danh sách và tìm kiếm thông tin học viên";
    ...
    hienThiDanhSachVienRows(listStudent, isEditingAllowed);
}

/**
 * Hiển thị các dòng (rows) dữ liệu sinh viên trong bảng
 */
function hienThiDanhSachVienRows(listStudent, isEditingAllowed) {
    let htmlStr = "";
    ...
    for (let i = 0; i < listStudent.length; i++) {
        ...
        let tenLop = lop ? lop.name : `<span style="color: var(--text-muted); font-style: italic;">Chưa phân lớp</span>`;
...
function locDanhSachSinhVien() {
    ...
    let currentUser = quanLyTaiKhoan.layNguoiDangNhap();
    let isEditingAllowed = currentUser && currentUser.role === "Admin";
    hienThiDanhSachVienRows(filtered, isEditingAllowed);
}
...
function hienThiFormTao(errorMsg = "") {
    ...
    document.getElementById("header-subtitle").textContent = "Tạo mới thông tin sinh viên";

    // Lấy danh sách lớp học để điền vào dropdown select
    let listLop = quanLyLop.layDanhSachLop();
    let classOptions = listLop.map(lop => `<option value="${lop.id}">${lop.name} (${lop.teacher})</option>`).join("");
...
function themSinhVien() {
    ...
    if (quanLy.kiemTraTrungId(id)) {
        hienThiFormTao(`Mã sinh viên "${id}" đã tồn tại trên hệ thống. Vui lòng chọn mã khác.`);
        return;
    }
...
function hienThiFormSua(id, errorMsg = "") {
    ...
    let listLop = quanLyLop.layDanhSachLop();
    let classOptions = listLop.map(lop => {
...
function capNhatSinhVien(id) {
    ...
    let age = parseInt(ageInput);
    if (isNaN(age) || age <= 0 || age > 100) {
        hienThiFormSua(id, "Tuổi sinh viên phải là số nguyên dương hợp lệ (1 - 100)");
        return;
    }
...
function xoaSinhVien(id) {
    let xacNhan = confirm(`Bạn chắc chắn muốn xóa sinh viên có ID là "${id}" không? Hành động này không thể hoàn tác.`);
    if (xacNhan) {
...
function hienThiDanhSachLop() {
    ...
    document.getElementById("header-subtitle").textContent = "Quản lý thông tin các lớp học hiện có";
    ...
    if (list.length === 0) {
        ...
        for (let i = 0; i < list.length; i++) {
            ...
            let siSo = allStudents.filter(student => student.classId == lop.id).length;
...
function hienThiFormThemLop(errorMsg = "") {
    ...
    document.getElementById("header-subtitle").textContent = "Tạo mới thông tin lớp học";
...
function xoaLop(id) {
    ...
    let isHavingStudents = quanLy.layDanhSachSinhVien().some(student => student.classId == id);
    if (isHavingStudents) {
        alert("Lưu ý: Không thể xóa lớp học này vì đang chứa học viên. Vui lòng chuyển học viên sang lớp khác trước khi xóa lớp.");
        return;
    }
...
function hienThiDangNhap(alertMsg = "", alertType = 'danger') {
    document.getElementById("header-subtitle").textContent = "Vui lòng đăng nhập tài khoản để vào hệ thống quản trị";
...
function hienThiDangKy(alertMsg = "", alertType = 'danger') {
    document.getElementById("header-subtitle").textContent = "Đăng ký tài khoản người dùng mới";
...
function dangKy() {
    ...
    if (check) {
        ...
        hienThiDangKy(`Tên đăng nhập "${username}" đã tồn tại. Vui lòng chọn tên khác.`);
    }
}
 ...
 */
function dangXuat() {
    let confirmLogout = confirm("Bạn có chắc chắn muốn đăng xuất khỏi hệ thống không?");
    if (confirmLogout) {
